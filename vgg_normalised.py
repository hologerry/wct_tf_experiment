...
from keras.layers import (Activation, Conv2D, Input, Lambda, MaxPooling2D)
...
def vgg_from_t7(t7_file, target_layer=None):
    ...
    for idx, module in enumerate(t7.modules):
        ...
        if idx == 0:
            # VGG 1st layer preprocesses with a 1x1 conv to multiply by 255 and subtract BGR mean as bias
            name = 'preprocess'
        ...
        if module._typename == b'nn.SpatialReflectionPadding':
            ...
            x = Conv2D(filters, kernel_size, padding='valid', activation=None, name=name,
                       kernel_initializer=lambda shape: K.constant(
                           weight, shape=shape),
                       bias_initializer=lambda shape: K.constant(
                           bias, shape=shape),
                       trainable=False)(x)
