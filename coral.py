...
def coral_numpy(source, target):
    ...
    source_flatten = source.reshape(
        n_channels, source.shape[1]*source.shape[2])
    target_flatten = target.reshape(
        n_channels, target.shape[1]*target.shape[2])

    source_flatten_mean = source_flatten.mean(axis=1, keepdims=True)
    source_flatten_std = source_flatten.std(axis=1, keepdims=True)
    source_flatten_norm = (
        source_flatten - source_flatten_mean) / source_flatten_std

    target_flatten_mean = target_flatten.mean(axis=1, keepdims=True)
    target_flatten_std = target_flatten.std(axis=1, keepdims=True)
    target_flatten_norm = (
        target_flatten - target_flatten_mean) / target_flatten_std

    source_flatten_cov_eye = source_flatten_norm.dot(
        source_flatten_norm.T) + np.eye(n_channels)
    target_flatten_cov_eye = target_flatten_norm.dot(
        target_flatten_norm.T) + np.eye(n_channels)

    source_flatten_norm_transfer = matSqrt_numpy(target_flatten_cov_eye).dot(
        np.linalg.inv(matSqrt_numpy(source_flatten_cov_eye))).dot(source_flatten_norm)
    source_flatten_transfer = source_flatten_norm_transfer * \
        target_flatten_std + target_flatten_mean
...
#     source_flatten_norm = (source_flatten - source_flatten_mean.expand_as(source_flatten)) \
#         / source_flatten_std.expand_as(source_flatten)

#     target_flatten_mean = target_flatten.mean(1).unsqueeze(-1) # 3x1
#     target_flatten_std = target_flatten.std(1).unsqueeze(-1)   # 3x1
#     target_flatten_norm = (target_flatten - target_flatten_mean.expand_as(target_flatten)) \
#         / target_flatten_std.expand_as(target_flatten)
...
#     source_flatten_norm_transfer = matSqrt_pytorch(target_flatten_cov_eye).mm(
#         torch.inverse(matSqrt_pytorch(source_flatten_cov_eye))).mm(source_flatten_norm)
#     source_flatten_transfer = source_flatten_norm_transfer * target_flatten_std.expand_as(source_flatten_norm) \
#         + target_flatten_mean.expand_as(source_flatten_norm)
