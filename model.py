...
class WCTModel(object):
    ...
    def __init__(self, mode='train', relu_targets=['relu5_1', 'relu4_1', 'relu3_1', 'relu2_1', 'relu1_1'],
                 vgg_path=None, *args, **kwargs):
        '''
            Args:
        ...
        self.style_input = tf.placeholder_with_default(tf.constant(
            [[[[0., 0., 0.]]]]), shape=(None, None, None, 3), name='style_img')
        ...
        if self.mode == 'train':
            # Style encoding is not needed for train stage
            ...
            with tf.name_scope('style_encoder'):
                style_encoding_layers = [self.vgg_model.get_layer(
                    relu).output for relu in relu_targets]
                style_encoder_model = Model(
                    inputs=self.vgg_model.input, outputs=style_encoding_layers)
        ...
        for i, (relu, style_encoded) in enumerate(zip(relu_targets, style_encodings)):
            ...
            enc_dec = self.build_model(
                relu, input_tensor=input_tensor, style_encoded_tensor=style_encoded, **kwargs)
    ...
    def build_model(self,
        ...
        '''Build the EncoderDecoder architecture for a given relu layer.
            ...
            Args:
                ...
                input_tensor: If None then a placeholder will be created,
                              else use this tensor as the input to the encoder
        ...
        with tf.name_scope('encoder_decoder_'+relu_target):
            ...
            with tf.name_scope('content_encoder_'+relu_target):
                if input_tensor is None:
                    ...
                    content_imgs = tf.placeholder_with_default(tf.constant(
                        [[[[0., 0., 0.]]]]), shape=(None, None, None, 3), name='content_imgs')
                else:
                ...
                content_encoder_model = Model(
                    inputs=self.vgg_model.input, outputs=content_layer)
            ...
            with tf.name_scope('decoder_'+relu_target):
                ...
                decoder_model = self.build_decoder(input_shape=(
                    None, None, n_channels), relu_target=relu_target)
        ...
        if self.mode == 'train':  # Train & summary ops only needed for training phase
            ...
            with tf.name_scope('losses_'+relu_target):
                ...
                feature_loss = feature_weight * \
                    mse(decoded_encoded, content_encoded)
                ...
                if tv_weight > 0:
                    tv_loss = tv_weight * \
                        tf.reduce_mean(tf.image.total_variation(decoded))
                else:
            ...
            with tf.name_scope('train_'+relu_target):
                global_step = tf.Variable(
                    0, name='global_step_train', trainable=False)
                # self.learning_rate = tf.train.exponential_decay(learning_rate, self.global_step,
                #                                                 100, 0.96, staircase=False)
                learning_rate = torch_decay(
                    learning_rate, global_step, lr_decay)
                d_optimizer = tf.train.AdamOptimizer(
                    learning_rate, beta1=0.9, beta2=0.999)

                # Only train decoder vars, encoder is frozen
                d_vars = [var for var in tf.trainable_variables(
                ) if 'decoder_'+relu_target in var.name]

                train_op = d_optimizer.minimize(
                    total_loss, var_list=d_vars, global_step=global_step)
            ...
            pixel_loss, feature_loss, tv_loss, total_loss,
            train_op, global_step, learning_rate, summary_op = [None]*8
    ...
    def build_decoder(self, input_shape, relu_target):
        '''Build the decoder architecture that reconstructs from a given VGG relu layer.
            ...
            Args:
                ...
                relu_target: Layer of VGG to decode from
        '''
        decoder_num = dict(zip(
            ['relu1_1', 'relu2_1', 'relu3_1', 'relu4_1', 'relu5_1'], range(1, 6)))[relu_target]
        ...
        for d in decoders:
            for layer_tup in decoder_archs[d]:
                ...
                if layer_tup[0] == Conv2DReflect:
                    x = Conv2DReflect(
                        layer_name, filters=layer_tup[1], kernel_size=3, padding='valid',
                        activation='relu', name=layer_name)(x)
        ...
        layer_name = '{}_{}'.format(relu_target, count)
        output = Conv2DReflect(layer_name, filters=3, kernel_size=3, padding='valid',
                               activation=None, name=layer_name)(x)  # 256x256 / 64->3
