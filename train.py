    ...
    while True:
        ...
        while idx < batch_shape[0]:  # Build batch sample by sample
            try:
                ...
                X_batch[idx] = get_img_random_crop(
                    f, resize=512, crop=256).astype(np.float32)
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        queue = tf.FIFOQueue(capacity=100, dtypes=[tf.float32, tf.float32], shapes=[
                             [256, 256, 3], [256, 256, 3]])
        ...
        with tf.Session(config=config) as sess:
            ...
            for iteration in range(args.max_iter):
                ...
                if iteration % args.summary_iter == 0:
                    ...
                    summary = sess.run(model.summary_op, feed_dict={
                                       model.content_input: val_batch})
                    summary_writer.add_summary(summary, results['global_step'])

                # Save checkpoint
                if iteration % args.save_iter == 0:
                    save_path = saver.save(sess, os.path.join(
                        args.checkpoint, 'model.ckpt'), results['global_step'])
                    print("Model saved in file: %s" % save_path)

                # Log training stats
                print("Step: {}  LR: {:.7f}  Feature: {:.5f}  Pixel: {:.5f}  TV: {:.5f}  Time: {:.5f}"
                      .format(results['global_step'], results['lr'], results['feature_loss'],
                              results['pixel_loss'], results['tv_loss'], time.time() - start))
